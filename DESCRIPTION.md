...
- **Steady Shot** (orange) — Shows when to weave Steady Shot between auto shots. At 1:1 speeds and faster the window is strictly clip-free; at slow (French-band) speeds it extends slightly past the no-clip deadline — by half the gap's idle GCD time, at most 0.4 s — because the Rotation Tools French rotation presses Steady at GCD boundaries even when the cast delays the auto by 0.12–0.36 s (its diagrams draw exactly those "autodelay" boxes; ~95% auto efficiency is the accepted price of ~96% GCD efficiency). When Multi-Shot is off cooldown the window takes Multi-Shot's color instead: press Multi-Shot there in place of the Steady (per Rotation Tools, a Multi-Shot should replace a Steady whenever it is off cooldown — its shorter cast always fits inside a steady window).
